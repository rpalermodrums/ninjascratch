...
@router.post('/todos', response=TodoOut, by_alias=True)
def create_todo(request, data: TodoIn) -> Todo:
    todo_list, created = TodoList.objects.get_or_create(id=getattr(data, 'list'), owner_id=request.user.id)
    todo = Todo.objects.create(**{**data.dict(), 'list': todo_list})
    return todo


@router.get('/todos', response=List[TodoOut], by_alias=True)
def list_todos(request, list_id: int = 1) -> List[Todo]:
    todo_list = get_object_or_404(TodoList, id=list_id)
    return todo_list.todo_items


@router.get('/todos/{todo_id}', response=TodoOut, by_alias=True)
...
@router.put('/todos/{todo_id}', response=TodoOut, by_alias=True)
def update_todo(request, todo_id: int, payload: TodoIn) -> Todo:
